...
class TLRI(observer.MetricObserver):
    ...
    def _get_initial_dataset(self, dss_instance: dss):
        ...
        self.xfmr_downward_customers = dss_util.get_transformer_customers(dss_instance)

    def compute(self, dss_instance:dss):
        """ Refer to base class for more details. """

        # Get voltage dataframe and load bus mapper
        xmfr_loading_df = powerflow_metrics_opendss.get_transloading_dataframe(dss_instance)
        ...
        merged_df = pd.merge(self.xfmr_downward_customers, ol_gamma, 
        ...
        if self.tlri_metric.empty:
            ...
            self.tlri_metric = pd.DataFrame(self.tlri_metric['metric'] + merged_df['metric'])
    ...
    def get_metric(self):
        """ Refer to base class for more details. """
        return {
                xfmrname: metric/self.counter if self.counter else 0 for xfmrname, metric \
                    in self.tlri_metric.to_dict().get('metric', {}).items()
        }
...
class LLRI(observer.MetricObserver):
    ...
    def compute(self, dss_instance:dss):
        ...
        merged_df = pd.merge(self.line_downward_customers, ol_gamma, 
            how="left", left_index=True, right_index=True).fillna(0)
    ...
    def get_metric(self):
        """ Refer to base class for more details. """
        return {
                linename: metric/self.counter if self.counter else 0 for linename, metric \
                    in self.llri_metric.to_dict().get('metric', {}).items()
        }
...
class NVRI(observer.MetricObserver):
    ...
    def compute(self, dss_instance:dss):
        ...
        merged_df_ = pd.merge(self.bus_load_flag_df, ov_gamma, 
            how="left", left_index=True, right_index=True).fillna(0)
        merged_df = pd.merge(merged_df_, uv_gamma, 
            how="left", left_index=True, right_index=True).fillna(0)
    ...
    def get_metric(self):
        """ Refer to base class for more details. """
        return {
                busname: metric/self.counter if self.counter else 0 for busname, metric \
                    in self.nvri_metric.to_dict().get('metric', {}).items()
        }
